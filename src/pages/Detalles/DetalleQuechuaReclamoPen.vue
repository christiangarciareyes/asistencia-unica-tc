  ...
  methods: {
     ...
     enviar (){
        if(this.atencion != ''){
          ...
          axios.post(this.$store.state.serverAsistenciaAdmin + '/updateReclamoQue2', fromData, {crossDomain: true, headers: {"Authorization": this.authorization}
          }).then((response) => {
            this.isLoading = false;
            this.atras();
            this.notifyVue4();
     ...
     derivando (){
        if(this.item.value != '' && this.derivacion2 != ''){
          ...
          fromData.append('iddetalletipoasistencia', this.idregistro);
          axios.post(this.$store.state.serverAsistenciaAdmin + '/updateReclamoQue', fromData, {crossDomain: true, headers: {"Authorization": this.authorization}
          }).then((response) => {
            this.isLoading = false;
            this.atras();
            this.notifyVue2();
     ...
     traduciendo(){
        if(this.traduccion2 != ''){
          ...
          fromData.append('cuenta', this.cuenta);
          fromData.append('iddetalletipoasistencia', this.idregistro);
          axios.post(this.$store.state.serverAsistenciaAdmin + '/traducirReclamoQue', fromData, {crossDomain: true, headers: {"Authorization": this.authorization}
          }).then((response) => {
            this.isLoading = false;
            this.atras();
            this.notifyVue5();
     ...
     notifyVue5() {
       this.$notify(
        {
          message: 'Reclamo enviado a traducir correctamente!',
          icon: 'add_alert',
          horizontalAlign: 'left',
          verticalAlign: 'top',
          type: 'success'
     ...
     notifyVue6() {
       this.$notify(
        {
          message: 'No se puede eliminar la traducción del reclamo!',
          icon: 'add_alert',
          horizontalAlign: 'left',
          verticalAlign: 'top',
          type: 'warning'
