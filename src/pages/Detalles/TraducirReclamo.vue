        ...
        <div class="col-lg-12">
          ...
          <div class="news_post">
            ...
            <div class="news_post_top d-flex flex-column flex-sm-row">
              <div class="news_post_title_container">
                <div class="news_post_title">
                  ...
                  <a style="color: #AF52BF">Traduce el Reclamo en Quechua</a>
                </div>
                <div class="news_post_meta">
                  <span class="news_post_author"><a style="color: #AF52BF">Traduce el envío del reclamo en quechua ingresado</a></span>
                </div>
              </div>
            </div>
            </div>
            ...
            <div class="news_post_text m-l-32 m-r-32">
              <p>En el siguiente formulario podrás traducir el reclamo que te ha sido enviado por fines de atención.</p>
            </div>
            <div class="pbar_container m-t-20 m-b-40 m-l-32 m-r-32">
                ...
                <label class="labelInput">TRADUCCIÓN DEL RECLAMO</label>
                <textarea id="text" class="form-control imp-fue mb-3"
                     style="border-radius: 0px; height:200px; max-height:200px; min-height:200px"
                     v-model="respuesta"
                     no-resize
  ...
  created() {
    if(localStorage.getItem('AccessToken') != null && localStorage.getItem('expires_at') != null && localStorage.getItem('profile') != null && localStorage.getItem('operaciones') != null && localStorage.getItem('nickname') != null){
      if(this.$store.state.reclamoQue == ''){
        ...
        this.idregistro = this.$store.state.reclamoQue.nid_detalletipoasistencia
        this.codreclamo = this.$store.state.reclamoQue.cnum_numperio
        this.fecreclamo = this.$store.state.reclamoQue.dfec_registro
        ...
        this.usuariotraductor = this.$store.state.reclamoQue.ctxt_usuariotraductor
        this.nomusuariotraductor = this.$store.state.reclamoQue.ctxt_nomusuariotraductor
        this.correotraductor = this.$store.state.reclamoQue.ctxt_correotraductor
        this.traduccion = this.$store.state.reclamoQue.ctxt_detalleresponsable2
        this.respuesta = this.$store.state.reclamoQue.ctxt_detalletraductor
        this.estado = this.$store.state.reclamoQue.nflg_estado
        this.usuariomodifica = this.$store.state.reclamoQue.cnom_usuariomodifica
        if(this.estado == 1){
          this.variante = true;
        }
      }     
  ...
  methods: {
    ...
    notifyVue() {
     ...
     notifyVue4() {
       this.$notify(
        {
          message: 'Reclamo en Quechua traducido correctamente!',
          icon: 'add_alert',
          horizontalAlign: 'left',
          verticalAlign: 'top',
          type: 'success'
