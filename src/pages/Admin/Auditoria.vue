              ...
              <div class="news_post_top d-flex flex-column flex-sm-row">
                <div class="news_post_title_container">
                  <div class="news_post_title">
                    <a style="color: #AF52BF">Auditoría de Reclamos, Solicitudes y Consultas</a>
                  </div>
                  <div class="news_post_meta">
                    <span class="news_post_author"><a style="color: #AF52BF">Gestiona los reclamos, solicitudes y consultas de la institución</a></span>
                  </div>
                </div>
              </div>
              <div class="news_post_text m-l-32">
                <p>En la siguiente ventana gestionarás todos los reclamos, solicitudes y consultas que hayan ingresado a la institución.</p>
              </div>
              ...
              <b-tabs class="m-t-40">
                ...
                <b-table bordered 
                    ...
                    <template slot="ver" slot-scope="row">
                      <button v-if="row.item.nid_tipoasistencia == 1" @click="verDoc(row.item)" type="button" class="btn-success-circle btn btn-xs" v-b-tooltip.hover title="Ver Reclamo" ><i class="fa fa-external-link p-r-3"></i></button>
                      <button v-if="row.item.nid_tipoasistencia == 2" @click="verDoc1(row.item)" type="button" class="btn-success-circle btn btn-xs" v-b-tooltip.hover title="Ver Solicitud" ><i class="fa fa-external-link p-r-3"></i></button>
                      <button v-if="row.item.nid_tipoasistencia == 3" @click="verDoc2(row.item)" type="button" class="btn-success-circle btn btn-xs" v-b-tooltip.hover title="Ver Solicitud" ><i class="fa fa-external-link p-r-3"></i></button>
                    </template>
                </b-table>
                <div class="row">
                  <div class="col-md-12 mb-3">
                      ...
                      <div class="container-form-btn m-t-10">
                        ...
                        <button v-if="solicitudes == true" class="routing2-form-btn m-r-10 m-l-10 fuente">
                          <download-excel
                            :data="json_data"
                            :fields="json_fields2"
                            worksheet="Auditoría - Solicitudes"
                            :name="nameExcel">
                            DESCARGAR
                          </download-excel>
                        </button>
...
export default{
  data () {
    return {
     ...
     fields: [
        ...
        { key: 'reclamante', label: 'DATOS DE LA PERSONA'},
      ...
      fields2: [
        ...
        { key: 'reclamante', label: 'DATOS DE LA PERSONA'},
        { key: 'registro', label: 'FECHA DE REGISTRO'},
        { key: 'registro2', label: 'FECHA DE ATENCIÓN'},
        { key: 'ver', label: 'VER'}
      ],
      lista: [],
      lista2: [],
  ...
  methods: {
    ...
    verDoc (item){
       ...
       this.$router.push('/auditoria/auditoriaReclamo');
    },
    verDoc1 (item){
       this.$store.commit('consulSolicitud', item);
       this.$router.push('/auditoria/auditoriaSolicitud');
    },
    verDoc2 (item){
      ...
      this.lista3.forEach( item =>{
        ...
        if(this.reclamos == true){
          ...
          this.json_data.push({
            ...
            "cnum_celular": item.cnum_celular,
            "cnum_expdiente": item.cnum_expdiente,
    ...
    notifyVue2() {
       ...
       this.$notify(
        {
          message: 'No existen atenciones pendientes!',
          icon: 'add_alert',
          horizontalAlign: 'left',
          verticalAlign: 'top',
          type: 'success'
    ...
    notifyVue3() {
       ...
       this.$notify(
        {
          message: 'No existen atenciones atendidas!',
          icon: 'add_alert',
          horizontalAlign: 'left',
          verticalAlign: 'top',
          type: 'success'
