              ...
              <div class="news_post_top d-flex flex-column flex-sm-row">
                <div class="news_post_title_container">
                  <div class="news_post_title">
                    <a style="color: #AF52BF">Administrador de Solicitudes</a>
                  </div>
                  <div class="news_post_meta">
                    <span class="news_post_author"><a style="color: #AF52BF">Gestiona tus solicitudes</a></span>
                  </div>
                </div>
              </div>
              <div class="news_post_text m-l-32">
                <p>En la siguiente ventana gestionarás todos las solicitudes de acceso a la información que hayan ingresado a la institución.</p>
              </div>
              ...
              <b-tabs class="m-t-40">
                ...
                <b-table bordered
                    ...
                    <template slot="ver" slot-scope="row">
                      <button @click="verDoc(row.item)" type="button" class="btn-success-circle btn btn-xs" v-b-tooltip.hover title="Ver Solicitud" ><i class="fa fa-external-link p-r-3"></i></button>
                    </template>
                    <template slot="hoja" slot-scope="row">
                      <button v-if="row.item.correo != '--'" @click="seguir(row.item)" type="button" class="btn-info-circle btn btn-xs" v-b-tooltip.hover title="Hoja de Seguimiento" ><i class="fa fa-binoculars p-r-3"></i></button>
                      <p v-if="row.item.correo == '--'" class="labelInput imp-fue">Sin Correo Electrónico</p>
                   </template>
                ...
                <b-table bordered
                    ...
                    <template slot="ver" slot-scope="row">
                      <button @click="verDoc2(row.item)" type="button" class="btn-success-circle btn btn-xs" v-b-tooltip.hover title="Ver Solicitud" ><i class="fa fa-external-link p-r-3"></i></button>
                    </template>
                </b-table>
                <div class="row">
                  <div class="col-md-12 mb-3">
                ...
                <b-table bordered
                    ...
                    <template slot="ver" slot-scope="row">
                      <button @click="verDoc1(row.item)" type="button" class="btn-success-circle btn btn-xs" v-b-tooltip.hover title="Ver Solicitud" ><i class="fa fa-external-link p-r-3"></i></button>
                    </template>
                    <template slot="hoja" slot-scope="row">
                      <button v-if="row.item.correo != '--'" @click="seguir(row.item)" type="button" class="btn-info-circle btn btn-xs" v-b-tooltip.hover title="Hoja de Seguimiento" ><i class="fa fa-binoculars p-r-3"></i></button>
                      <p v-if="row.item.correo == '--'" class="labelInput imp-fue">Sin Correo Electrónico</p>
                ...
                <b-table bordered
                    ...
                    <template slot="ver" slot-scope="row">
                      <button @click="verDoc3(row.item)" type="button" class="btn-success-circle btn btn-xs" v-b-tooltip.hover title="Ver Solicitud" ><i class="fa fa-external-link p-r-3"></i></button>
                    </template>
                </b-table>
                <div class="row">
                  <div class="col-md-12 mb-3">
...
export default{
  data () {
    return {
     ...
     fields: [
        ...
        { key: 'numero', label: 'N° DE SOLICITUD'},
  ...
  methods: {
    ...
    verDoc (item){
       ...
       this.$router.push('/solicitudes/detalleSolicitudPen');
    },
    verDoc1 (item){
       this.$store.commit('consulSolicitud', item);
       this.$router.push('/solicitudes/detalleSolicitudAte');
    },
    verDoc2 (item){
       this.$store.commit('consulSolicitudQue', item);
       this.$router.push('/solicitudes/detalleQuechuaSolicitudPen');
    },
    verDoc3 (item){
       this.$store.commit('consulSolicitudQue', item);
       this.$router.push('/solicitudes/detalleQuechuaSolicitudAte');
    },
    cargaView(){
      this.isLoading = true;
    ...
    alertar(datos){
        ...
        fromData.append('pais', datos.ctxt_pais);
        fromData.append('departamento', datos.ctxt_departamento);
        fromData.append('distrito', datos.ctxt_distrito);
        fromData.append('avenida', datos.ctxt_avenida);
        fromData.append('interior', datos.ctxt_interior);
        fromData.append('urbanizacion', datos.ctxt_urbanizacion);
        fromData.append('correo', datos.ctxt_correo);
        fromData.append('celular', datos.cnum_celular);
        fromData.append('tema', datos.ctxt_tema);
        fromData.append('dependencia', datos.ctxt_dependencia);
        fromData.append('formaentrega', datos.ctxt_formaentrega);
        fromData.append('especificacion', datos.ctxt_especificacion);
        fromData.append('solicitud', datos.ctxt_solicitud);
        fromData.append('observaciones', datos.ctxt_observaciones);
    ...
    consultar1(param){
      ...
      this.isLoading = true;
      axios.get(this.$store.state.serverAsistenciaAdmin + '/listSolicitudesPendientes?periodo=' + anio, {crossdomain: true,
      headers: {
        Authorization: this.authorization
      }
      }).then((response) => {
    ...
    consultar2(param){
      ...
      this.isLoading = true;
      axios.get(this.$store.state.serverAsistenciaAdmin + '/listSolicitudesAtendidas?periodo=' + anio, {crossdomain: true,
      headers: {
        Authorization: this.authorization
      }
      }).then((response) => {
    ...
    consultar3(param){
      ...
      this.isLoading = true;
      axios.get(this.$store.state.serverAsistenciaAdmin + '/listQuechuaSolicitudesPendientes?periodo=' + anio, {crossdomain: true,
      headers: {
        Authorization: this.authorization
      }
      }).then((response) => {
    ...
    consultar4(param){
      ...
      this.isLoading = true;
      axios.get(this.$store.state.serverAsistenciaAdmin + '/listQuechuaSolicitudesAtendidas?periodo=' + anio, {crossdomain: true,
      headers: {
        Authorization: this.authorization
      }
      }).then((response) => {
    ...
    generar(){
      if(this.aniobus !== "" && this.aniobus !== null){
        ...
        this.isLoading = true;
        axios.get(this.$store.state.serverAsistenciaAdmin + '/listReporteSolicitudes?anio=' + anio + '&mes=' + mes + '&tipo=' + tipo,  {crossdomain: true,
        headers: {
          Authorization: this.authorization
        }
        }).then((response) => {
    ...
    notifyVue2() {
       ...
       this.$notify(
        {
          message: 'No tiene solicitudes pendientes!',
          icon: 'add_alert',
          horizontalAlign: 'left',
          verticalAlign: 'top',
          type: 'success'
    ...
    notifyVue3() {
       ...
       this.$notify(
        {
          message: 'No tiene solicitudes atendidas!',
          icon: 'add_alert',
          horizontalAlign: 'left',
          verticalAlign: 'top',
          type: 'success'
    ...
    notifyVue7() {
       ...
       this.$notify(
        {
          message: 'No tiene solicitudes en quechua pendientas!',
          icon: 'add_alert',
          horizontalAlign: 'left',
          verticalAlign: 'top',
          type: 'success'
    ...
    notifyVue8() {
       ...
       this.$notify(
        {
          message: 'No tiene solicitudes en quechua atendidas!',
          icon: 'add_alert',
          horizontalAlign: 'left',
          verticalAlign: 'top',
          type: 'success'
