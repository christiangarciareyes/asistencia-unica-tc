					 ...
					 <transition name="fade" mode="out-in">
						<div class="news_post" v-if="macro">
							<div class="news_post_top d-flex flex-column flex-sm-row">
								<div class="news_post_title_container3">
									<div class="news_post_title">
										<a style="color: #AF52BF">Consulta el Estado de tu Reclamo</a>
									</div>
							...
							<div class="news_post_text">
								<p>En el siguiente formulario registrará su número de documento y/o su número de reclamo para buscar y verificar el estado de su reclamo que ingresó anteriormente.</p>
							</div>
							<div class="pbar_container m-t-20">
							  <div class="row">
								...
								<div class="col-md-6">
								   <label class="labelInput">NÚMERO DE RECLAMO</label>
						...
						<transition name="fade" mode="out-in">
                         <detalle-reclamo v-if="micro"></detalle-reclamo>
                        </transition>
					</div>
				</div>
			</div>
...
export default {
  ...
  methods:{
      ...
      if(this.dni != '' && this.numero != ''){
	      ...
	      axios.get(this.$store.state.serverAsistenciaPublic + '/listReclamosPublic?cnum_numperio=' + this.numero, {crossdomain: true
	      }).then((response) => {
	      		this.lista = response.data;
	      		this.lista = this.lista.response;
	      		if(this.lista != ''){
